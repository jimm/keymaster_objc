...
KeyMaster save feature isn't all that useful since you can't change
anything from within KeyMaster anyway --- yet.
...
While running KeyMaster, the 'e' key lets you edit the file you loaded or
most recently saved. If you have not yet loaded a file or you save the file
to a different location, KeyMaster will ask you for a file name.

The edit command suspends KeyMaster and starts the editor defined by the
environment variables =VISUAL= or =EDITOR=. If neither of those are defined
it tries 'vim', 'vi', and finally 'notepad.exe'.

After editing a file, KeyMaster attempts to reload that file and continue
with the same song list, song, and patch that was current when you edited
the file.

When you edit a file from within KeyMaster, it has no way of knowing if
...
KeyMaster file requires those blocks be saved. However, there is no
practical way to obtain the text of a code block across all versions of Ruby
at runtime. To get around this, when loading the file KeyMaster re-reads
...
2. If your KeyMaster file creates triggers or filters in a loop (more
...
3. If your filter or trigger does anything tricky like changing its own code
   (replacing its block with another) there is no way that KeyMaster can
   know the text of the new code. When the trigger or filter is saved, the
   old block text will be written out.

As a workaround, you'll have to avoid using KeyMaster's save feature. Make
all your edits to triggers and filters from outside of KeyMaster, using
your editor.

To be honest, the KeyMaster save feature isn't all that useful since you
can't change anything from within KeyMaster anyway --- yet. Once editing
capabilities are added to KeyMaster this might become more bothersome.
...
Maps the named message to a key. Message keys are ignored if KeyMaster was
...
of PM::KeyMaster are made available to the trigger, so for example the
...
#+begin_src ruby
  chain name, [song_name, song_name...]
#+end_src
